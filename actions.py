...
class Actions(Utils):
    def test_run(self, node_ip, port, id, test_script, db_run):
    ...
    def test_black(self, node_ip, port, id, db_run):
    ...
    def build(self, install_script, id, db_run):
        if install_script:
    ...
    def build_and_test(self, id):
        ...
        uuid, response, node_ip, port = self.build(install_script=install_script, id=id, db_run=RepoRun)
        if uuid:
            if not response.returncode:
                self.test_black(node_ip=node_ip, port=port, id=id, db_run=RepoRun)
                self.test_run(node_ip=node_ip, port=port, id=id, test_script=test_script, db_run=RepoRun)
                self.cal_status(id=id, db_run=RepoRun)
            vms.destroy_vm(uuid)
        self.report(id=id)

    def run_project(self, project_name, install_script, test_script):
        status = "pending"
        project_run = ProjectRun(timestamp=datetime.now().timestamp(), status=status, name=project_name)
        project_run.save()
        id = str(project_run.id)
        uuid, response, node_ip, port = self.build(install_script=install_script, id=id, db_run=ProjectRun)
        if uuid:
            if not response.returncode:
                self.test_run(node_ip=node_ip, port=port, id=id, test_script=test_script, db_run=ProjectRun)
        ...
        if project_run.status == "success":
            ...
            self.send_msg("{} tests errored {}".format(project_name, self.serverip))
