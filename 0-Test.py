...
def test_run(image_name, branch, commit, committer):
    ...
    if content:
        ...
        for line in lines:
            response = test.run_tests(image_name=image_name, run_cmd=line)
...
def start_test(branch, commit, committer):
...
def triggar(**kwargs):
    ...
    if request.json:
        ...
        if request.json.get('ref'):
            ...
            start_test(branch=branch, commit=commit, committer=committer)
    return "Done", 201
