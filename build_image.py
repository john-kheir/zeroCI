...
class BuildImage(Utils):
    def image_bulid(self, image_name, file="Dockerfile", branch="development", commit=""):
        ...
        :type commit: str
        """
        cmd = "docker build --force-rm -t {} . -f {} --build-arg branch={} --build-arg commit={}".format(
            image_name, file, branch, commit
        )
        response = self.execute_cmd(cmd, timeout=1800)
        return response
